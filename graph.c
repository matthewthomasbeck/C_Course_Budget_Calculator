...
pqType createPQtype( graphType gt, double priority );
...
Graph* createGraph( int capacity )
{
    ...
    g->adjacencyMatrix = (double**)malloc(sizeof(double*)*capacity);
    g->pathsArray = (int**)malloc(sizeof(int*)*capacity);
    g->distance = (double**)malloc(sizeof(double*)*capacity);

    for( i=0; i<capacity; i++ ){
        g->adjacencyMatrix[i] = (double*)malloc(sizeof(double)*capacity);
        g->pathsArray[i] = (int*)malloc(sizeof(int)*capacity);
        g->distance[i] = (double*)malloc(sizeof(double)*capacity);
    }
...
/* setEdge
 * input: a pointer to a Graph, two graphType variables, a double
 * output: none
 *
 * Sets the specified edge in the graph to the double value
 * You can pass value=-1 to remove the edge
 */
void setEdge( Graph* g, graphType p1, graphType p2, double value ) // p1 and p2 are 2 different vertices, and the edge is the weight between them (adjacent distance from one and the other)
{
    int i, j; // i is index for p1, j is for p2
 ...
 * output: a double
 *
 * Returns the double value for the specified edge is in the graph or -1 if the edge has not been added
 */
...
/* setDistance
 * input: a pointer to a Graph, two graphTypes, a double
 * output: none
 *
 ...
 * Remember dijkstrasAlg sets this to INT_MAX if no path exists.
 */
    ...
    while( !isEmptyPQ(q) ){
        ...
        if( !getVisited( g, cur.info ) ){
            ...
            while( getSuccessor(g, cur.info, &next) ) {;
                if( !getVisited( g, next ) && getDistance( g, start, next ) > 
                                              getDistance( g, start, cur.info )+getEdge( g, cur.info, next )){
